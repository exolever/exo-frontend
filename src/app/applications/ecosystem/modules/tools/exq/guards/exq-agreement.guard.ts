...
@Injectable()
export class ExqAgreementGuard implements CanActivateChild {
  ...
  private checkPermissions (nextUrl: string): Observable<boolean> {
    return this.store.pipe(
      ...
      map((user: UserModel) => {
        if (user.hasPermissionsSuperuserIncluded(StaticPermissions.CAN_VIEW_EXQ)) {
          return true;
        } else {
          this.router.navigate([Urls.ECOSYSTEM_EXQ_CONDITIONS], { state: {nextUrl: nextUrl}});
          return false;
        }
      })
    );
