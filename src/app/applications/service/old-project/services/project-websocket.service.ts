...
export class ProjectWebsocketService {
  ...
  subscribe(projectPk) {
    this.socket.emit(WsEvents.SUBSCRIBE, WsSubscriptionsName.PROJECT, [projectPk]);
    this.socket.addConnectedWs({name: WsSubscriptionsName.PROJECT, parameters: projectPk});
  }

  getMessages() {
    return this.socket.messages$.pipe(
      map(message => JSON.parse(message)),
      filter((message: WebSocketPayload) => message.subscription === WsSubscriptionsName.PROJECT),
    ...
    switch (objectName) {
      case WsProjectObjects.MICROLEARNING_AVERAGE:
        if (WsProjectActions.UPDATE === actionName) {
          ...
          this.store.dispatch(new StepActions.UpdateQuizRatings(payload));
        }
        break;
