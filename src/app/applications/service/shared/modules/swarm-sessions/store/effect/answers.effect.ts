...
export class AnswersEffect {
  ...
  @Effect()
  rating$: Observable<Action> = this.actions$
    .pipe(
      ofType(AnswersActions.TypeActionEnum.RATE_ANSWER),
      switchMap((action: AnswersActions.RateAnswer) => this.sessionService.rateAnswer({
        pkTeam: action.payload.pkTeam,
        pkProject: action.payload.pkProject,
        pkSession: action.payload.pkSession,
        pkQuestion: action.payload.pkQuestion,
      ...
      catchError(error => this.notify('WRONG', new AnswersActions.RateAnswerFail(error), true)),
